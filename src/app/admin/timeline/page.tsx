...
import { getHiddenEvents, getTimelineRows } from "@/lib/db";

// 편집 「사건」 탭. 후보 연결선까지 포함해 가져오고, 사건 추가·수정·숨김과 메모 편집 UI가 열린다.
...
export default async function AdminTimelinePage() {
  const [{ rows, segments }, hiddenEvents] = await Promise.all([
    getTimelineRows({ includeCandidates: true }),
    getHiddenEvents(),
  ]);
  ...
  return (
    <>
      ...
      <TimelineExperience rows={rows} segments={segments} mode="admin" />
