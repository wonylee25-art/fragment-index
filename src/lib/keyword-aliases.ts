...
//   1. 별칭 표에 적혀 있으면 그것
//   2. 아니면 공백을 지운 형태가 같은 것들 중 편수가 가장 많은 표기
...
export function buildCanonicalMap(papers: PaperData[]): Map<string, string> {
  ...
  // 별칭 표를 먼저 적용해 두고, 남은 것끼리 띄어쓰기로 묶는다. 별칭의 대표어가 또 다른
  // 표기와 띄어쓰기만 다를 수 있으므로(농촌여성노인 → 농촌 여성 → 농촌여성) 두 단계를 잇는다.
  const aliased = new Map<string, string>();
  for (const keyword of order) {
    aliased.set(keyword, KEYWORD_ALIASES[keyword] ?? keyword);
  }

  // 띄어쓰기 덩이마다 대표 표기를 고른다. 별칭의 대표어는 원본에 없을 수도 있어 함께 센다.
  const best = new Map<string, string>();
  for (const keyword of order) {
    const target = aliased.get(keyword)!;
    const key = spacingKey(target);
    const current = best.get(key);
    if (!current || (counts.get(target) ?? 0) > (counts.get(current) ?? 0)) {
      best.set(key, target);
    }
  }
