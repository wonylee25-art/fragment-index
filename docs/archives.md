...
#### 2026-08-19 추가 확인 — 웹 검색화면 쪽 내부 API는 토큰 벽에 막힘
- 키 없이 웹 검색화면(`www.dbpia.co.kr`)을 긁을 수 있는지 다시 확인함. **결론은 ✗ — 자동 수집 불가.**
- `robots.txt`는 오히려 관대함: 기본은 `Disallow: /`지만 검색엔진·AI 봇을 이름으로 열어주고 **`Claude-User`도 `Allow: /`, `Crawl-delay: 5`**로 명시돼 있음. 막는 건 정책이 아니라 아래 인증 방식.
- 검색 결과는 서버 렌더링이 아니라 `POST https://www.dbpia.co.kr/api/search/list`(JSON)로 채워짐. 요청 본문에 `query`·`page`·`pageSize`·`prefix`(검색 필드)·`subjectCodes`(주제분류)·`sort` 등이 들어가고, 응답에는 제목·저자·소속·학술지·권(호)·발행연월·주제어·**초록**·이용수까지 있어 RISS보다 데이터가 좋음(상세페이지 재요청이 필요 없음).
- 그런데 이 POST가 **페이지 로드마다 발급되는 일회성 토큰 헤더 `X-JSP-KEY`**를 요구함. 검색 페이지 HTML의 `searchResultKey = '...'` 변수에 그 값이 들어 있지만, 그걸 그대로 써도 `405`가 떨어짐(토큰 없이 보내도 같은 `405`). 스크립트에서 직접 호출하는 길은 여기서 끊김.
- 참고용 파라미터: 검색 필드는 URL `searchOption=` / 본문 `prefix`로 `all`·`node`(논문명)·`nodeAndAbstract`·`author`·`keyword`. 주제분류 코드는 `NA`(인문학)·`NB`(사회과학)·`NG`(예술체육학)·`NH`(복합학) 등.
- 건수(2026-08-19): 전체검색 "구술사" 10,290건이지만 그중 예술체육학이 7,628건(미술 7,430건)이라 노이즈가 압도적. **논문명 검색으로 좁히면 "구술사" 797건, "생애사" 1,039건.**
- 그래서 이날 하려던 "DBpia 학술논문을 연구동향에 추가"는 **RISS 수집 범위를 넓히는 쪽으로 대체**함(아래 RISS 항목 참고). DBpia는 여전히 OpenAPI 키를 받은 뒤에나 열리는 길이고, 사용자 확인 결과 2026-08-19 시점에는 키 발급이 안 되는 상태.
...
- **2026-08-19 범위 확대**: 학술논문을 *구술사연구*·한국구술사학회 학술대회 두 발행물로만 받던 제한을 풀었다(DBpia 쪽이 토큰 벽에 막혀 같은 목적을 RISS로 달성). 새 조건은 **제목에 "구술" 또는 "생애사"가 들어갈 것 + 발행 학술지·학회가 예술체육·의약학·공학·종교 계열이 아닐 것**(`isCollectedArticle`). RISS 정확검색은 초록·주제어까지 걸려서 DBpia의 "논문명" 검색에 해당하는 좁힘을 제목 조건으로 따로 걸었고, RISS 목록에는 주제분류가 없어 분야 제한은 학술지·학회 이름으로 근사한다. 목록만 미리 훑어 계산한 결과 학술논문 대상이 145건 → 1,094건(학술지 410종)으로 늘어남 — 상세페이지 요청이 건당 10초라 첫 실행은 2~3시간 걸린다.
