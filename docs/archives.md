...
- **메타데이터+영상URL만 제공, 재호스팅 없음** — `item_type: "구술"`로 `archive_items`에 저장, `description`엔 구술 요약 텍스트(150자 절삭) 저장.
- **`source_url`은 유튜브가 아니라 전시관 원문 페이지**(2026-08-19 변경). API가 주는 유튜브 링크가 제목과 어긋나는 자료가 있다 — "닭똥까지 주워 먹었던 보릿고개 이야기"(`.042`, 비공개인 듯)에 "한 땀 한 땀 자긍심을 바느질하다"(`.041`)의 영상이 붙어 있고, 전시관 게시판 원본도 같은 영상이라 **상류 데이터 오류**라 우리 쪽에서 고칠 수 없다. 나머지 63건은 제목과 영상이 맞음(유튜브 oEmbed 제목으로 64건 전수 대조).
- 그래서 전시관 "구술영상기록" 게시판(`/cop/bbs/selectBoardList.do?bbsId=tell_info_main&menuNo=050300`, 64건으로 API와 건수 일치)을 훑어 `nttId`·제목·영상ID 대응표를 `src/lib/womens-oral-links.json`에 만들어 두고(`scripts/build-womens-oral-links.mjs`), 제목으로 맞춘 상세 페이지 `…/selectBoardArticle.do?bbsId=tell_info_main&menuNo=050300&nttId=<id>`를 링크로 쓴다. 상세는 원래 POST 폼으로 열리지만 같은 파라미터를 GET으로 붙여도 열린다(전수 확인).
- 제목이 똑같은 자료가 6건("대한민국 건국훈장 애국장을 받은 여성독립유공자") 있어 제목 매칭만으로는 못 가르고, 영상 ID로 갈라 붙였다. 64건 전부 상세 페이지로 매칭됨.
- **`PageLink.do`는 링크로 못 쓴다** — 메뉴 이동용 POST 폼 엔드포인트라 GET으로 열면 오류 페이지로 넘어간다.
