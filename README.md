...
| 오른쪽 노란 획 | 내가 켠 것 — "여기는 더 볼 일 없다" | 노랑 | 예 |
...
두 축은 `사업 개요` 6칸(ISAD 3.2~3.3)과 `활용정책` 9칸(3.4)입니다. **활용정책은 이 문서의 가장 큰
구멍입니다** — 52건 가운데 아홉 칸을 들여다본 것이 15건뿐이고, 그중에서도 아홉 칸을 다 채운 사업은
하나도 없습니다. 화면 맨 아래 대장에서 축을 `활용정책`으로 갈면 그 구멍이 한눈에 보입니다.
