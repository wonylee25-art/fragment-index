...
| `/oral-history-projects` | 구술 사업 (탭 셋) | 국내 구술채록 사업 117건. `기록물 박스`는 주제 카테고리 여섯의 113건을 **ISAD(G) 계열 기술 라벨이 붙은 상자**로 세우고 카테고리마다 선반 하나를 준다(누르면 첫 자리로 나오며 옆에 기술지가 펴진다). `기록물 대장`은 같은 113건을 눕혀 한 칸을 세로로 견주고, `수행기관`은 축이 다른 4건을 표로 세운다(아래 참고) |
...
**말은 두 벌로 씁니다 — 규칙어와 카테고리.** 층위는 규칙어 그대로입니다: ISAD(G) 2판의 fonds → series →
file → item을 국가기록원 기술 규칙의 우리말로 옮긴 **기록물군 → 계열 → 철 → 건**이고, 사업 하나가 곧
**계열**입니다. 반면 선반을 가르는 **1~6·A는 층위가 아니라 분류**입니다 — 원칙대로면 1~6은 주제로 묶은
**주제군**이고 A는 주제가 아니라 **출처·행위자 축**이라, 둘을 한 번에 세는 규칙어가 애초에 없습니다.
그래서 묶음 쪽만 규칙어가 아닌 **카테고리**로 부릅니다(규칙 밖의 것에 규칙어를 갖다 쓰면 층위와
헷갈립니다 — 기술 칸의 `L1`·`L2`와 같은 처리입니다). **층위는 규칙어로, 묶음은 카테고리로.**
...
서는데, **그 빈칸이 곧 다음 조사 목록입니다** — 2026-08-28에 계열이 117건으로 늘어난 뒤 다시 세면
「권리 귀속」은 **117건 중 확인 0건**(안 봄 96 · 못 찾음 13 · 일부 8)이고, 「범위와 내용」은
**50건이 「일부」**(소재만 있고 시대·지역이 빔), 활용 「공개」는 **57건이 안 봄**입니다. 계열이
갑절이 되는 동안 권리 귀속은 한 건도 안 찼습니다 — **기관이 밖에 안 내놓는 칸**이라는 뜻입니다.
...
**수행기관을 서가에서 뺀 이유(2026-08-28)**: 카테고리 A는 나머지와 축이 다릅니다 — 1~6번은 "이 기관이
이런 사업을 벌였다"는 주제 묶음이고, A는 "남의 사업을 대신 채록한 쪽"이라 출처·행위자 축입니다. 같은
선반에 얹으면 상자 넷이 주제 하나인 것처럼 보입니다. 상자가 아니라 **표**인 것도 그래서입니다 —
이쪽을 읽는 물음은 "이 사업이 무엇인가"가 아니라 **"누구 밑에서 몇 건을 했나"**여서, 넷을 나란히
눕혀야 답이 보입니다. 하위구분(수탁형/과제형)과 수주 건수가 한 열로 서면 둘이 짝을 이루는 것이 그대로
드러납니다 — **수탁형만 수주 건수가 있습니다.** 줄을 누르면 상자와 같은 기술지가 그 자리에서 펴집니다.

**대장이 따로 있는 이유**: 서가는 하나를 고르는 자리라 상자마다 제 라벨을 들고 서 있고 같은 칸이
세로로 서지 않습니다. "누가 열람절차(3.4.1)를 확인했나"처럼 한 요소를 113건에 걸쳐 훑으려면 눕힌
표가 따로 있어야 합니다.

**둘을 쌓다가 탭으로 갈랐습니다(2026-08-28)**: 본래는 갈아 끼우면 둘 중 하나가 늘 숨는다는 이유로
세로로 쌓았는데, 계열이 117건이 되자 카테고리 일곱 선반을 다 지나야 대장에 닿았습니다. 지금은
**편집 화면(`AdminTabs`)과 같은 꼴의 탭 띠**를 머리글 바로 밑에 깔고 `기록물 박스`·`기록물 대장`·
`수행기관` 셋을 갈아 끼웁니다 — 숨는 값은 치르되 탭이 켜져 있어 어느 보기인지는 눈에 남습니다. 검색은 두 보기가
나눠 쓰므로 탭 아래 머리줄에 두고, **대장에서 한 줄을 누르면 그 계열이 켜진 채 서가로 건너갑니다**
(기술지가 펴지는 곳은 서가입니다). 문서 끝의 「확인했으나 정보 불충분」·「다음으로 고려할 것」 두
접힘도 대장 쪽에만 답니다 — 서가는 세워 둔 상자만 이는 자리고, 못 세운 것을 대는 자리는 대장입니다.
...
- **참조코드가 문서 순서에 매여 있습니다** — `KR-OHP-{카테고리}.{생산자}.{계열}`의 가운데 자리가 기관이
...
- **카테고리가 계열을 못 따라갑니다** — 2026-08-27~28에 계열이 56건에서 **117건**으로 늘면서, 들어갈
  카테고리가 없어 남의 선반에 얹어 둔 항목이 스물을 넘었습니다(과학기술사·법조·디자인공예·외교·도서관·
  재외동포·분단·질병 격리 피해·해녀·미군기지·팬데믹 등). 새 카테고리에 번호를 붙이면 문서의 8-1~8-9
  라벨과 부딪혀 문서 여러 곳을 함께 손봐야 하므로 아직 미룹니다 —
  [docs/oral_history_projects.md](docs/oral_history_projects.md)의 「다음으로 고려할 것」 4번.
...
실데이터는 **Supabase**(Postgres)에 저장되어 있고, 원본 소스는 두 곳입니다.
...
노란 획은 문서에 없던 조사자의 작업 상태라 겹칠 것이 없어, `is_important`·`highlighted`와 같은 층으로
