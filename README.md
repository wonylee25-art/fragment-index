...
| `/oral-history-projects` | 구술 사업 (서가) | 국내 구술채록 사업 52건을 **ISAD(G) 계열 기술 라벨이 붙은 상자**로 세우고, 갈래마다 선반 하나를 준다. 상자를 누르면 그 상자가 첫 자리로 나오며 옆에 기술지가 펴진다. 화면 맨 아래에는 같은 52건을 눕힌 **기록물 대장**이 있다(아래 참고) |
...
문서가 아직 옛 육하원칙으로 적혀 있는 항목은 파서가 여섯 칸을 새 칸으로 옮겨 채웁니다. 대응이
없는 칸(규모·행정연혁·권리 귀속·활용 아홉 등)은 `· 아직 안 봄`으로 서는데, **그 빈칸이 곧 다음
조사 목록입니다.**
...
세로로 서지 않습니다. "누가 열람절차(3.4.1)를 확인했나"처럼 한 요소를 52건에 걸쳐 훑으려면 눕힌 표가
...
- **규모(3.1.5)가 늘 "미기재"** — 문서에 `- **규모**:` 필드가 없습니다. 넣으면 라벨이 바로 채웁니다.
...
- **계열 정제가 남았습니다** — 제목에 `/`로 사업 여럿을 이어붙인 항목이 10건 있습니다. 가르는 기준은
  [docs/oral_history_projects.md](docs/oral_history_projects.md)의 「기술 단위 — 사업마다 한 계열」에
  확정해 두었고, 판정표는 `public/_demo-oral-split.html`에 있습니다. 가르면 52 → 최소 55건이 됩니다.
