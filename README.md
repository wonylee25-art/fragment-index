...
| `/oral-history-projects` | 구술 사업 (서가·대장) | 국내 구술채록 사업 117건을 **ISAD(G) 계열 기술 라벨이 붙은 상자**로 세우고, 카테고리마다 선반 하나를 준다. 상자를 누르면 그 상자가 첫 자리로 나오며 옆에 기술지가 펴진다. 같은 117건을 눕힌 **기록물 대장**은 편집 화면과 같은 꼴의 탭으로 갈라 둔다(아래 참고) |
...
세로로 서지 않습니다. "누가 열람절차(3.4.1)를 확인했나"처럼 한 요소를 117건에 걸쳐 훑으려면 눕힌
표가 따로 있어야 합니다.

**둘을 쌓다가 탭으로 갈랐습니다(2026-08-28)**: 본래는 갈아 끼우면 둘 중 하나가 늘 숨는다는 이유로
세로로 쌓았는데, 계열이 117건이 되자 카테고리 일곱 선반을 다 지나야 대장에 닿았습니다. 지금은
**편집 화면(`AdminTabs`)과 같은 꼴의 탭 띠**를 머리글 바로 밑에 깔고 `기록물 박스`·`기록물 대장`을
갈아 끼웁니다 — 숨는 값은 치르되 탭이 켜져 있어 어느 보기인지는 눈에 남습니다. 검색은 두 보기가
